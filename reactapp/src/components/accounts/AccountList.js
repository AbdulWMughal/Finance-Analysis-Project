    ...
    const rows = accountList.map((element) => {
                    ...
                    <Grid>
                        <Grid.Col span={"content"}>
                            ...
                            <Badge variant="filled" size={"xl"}><Text
                                         fw={700}>£. {element.currentBalance.toLocaleString('en-US')}</Text></Badge>
                        </Grid.Col>
                    </Grid>
                    <Grid>
                        <Grid.Col style={{marginLeft: 'auto'}} span={"content"}>
        ...
        return (
            <tr key={element.accountId}>
                ...
                <td>
                    <Text fw={700}>{`£. ${element.totalIncome.toLocaleString('en-US')}`}</Text>
                </td>
                <td>
                    <Text fw={700}>{`£. ${element.totalExpenses.toLocaleString('en-US')}`}</Text>
                </td>
                <td>
                    <Text fw={700} style={{color: '#26AB35'}}>
                        {`£. ${element.currentBalance.toLocaleString('en-US')}`}
                    </Text>
                </td>
