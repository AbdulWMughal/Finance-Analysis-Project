    ...
    const rows = budgetList.map((element) => {
                ...
                <Grid>
                    ...
                    <Grid.Col span={10} >
                        ...
                        <Progress
                            ...
                            size="sm"
                        />
                        <Text style={{marginTop:8,fontSize:10}}>{`Balance left to spend  £. ${element.balance.toLocaleString('en-US')} in £. ${element.amount.toLocaleString('en-US')}`}</Text>
                    </Grid.Col>
                </Grid>
            </div>
        );
        ...
        return (
            <tr key={element.budgetId}>
                ...
                <td>
                    <Text fw={700}>{`£. ${element.amount.toLocaleString('en-US')}`}</Text>
                </td>
                <td>
                    <Grid>
                        <Grid.Col span="content">
                            <Text fw={700}>{`£. ${element.used.toLocaleString('en-US')}`}</Text>
                        </Grid.Col>
                        <Grid.Col span="auto">
                            <Progress
                                tooltip={(100 * element.used) / element.amount}
                ...
                <td>
                    <Text fw={700} style={{color: '#26AB35'}}>
                        {`£. ${element.balance.toLocaleString('en-US')}`}
                    </Text>
                </td>
